...
class TestBattleEngine extends AnyFlatSpec with should.Matchers:
  ...
  val slowestBt: Turn = Turn(player.id, bulbasaur, BattleTurnEvent.Attack(actionMove))
  val fastestBt: Turn = Turn(opponent.id,charmender, BattleTurnEvent.Attack(actionMove))
  ...
  val lethalBt: Turn = Turn(player.id, bulbasaur , BattleTurnEvent.Attack(lethalMove))
