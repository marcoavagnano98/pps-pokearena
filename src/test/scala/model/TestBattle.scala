...
class TestBattle extends AnyFlatSpec with should.Matchers:
  ...
  val slowestBt: BattleUnit = BattleUnit(player.id, bulbasaur, BattleChoice.Attack(actionMove))
  val fastestBt: BattleUnit = BattleUnit(opponent.id,charmender, BattleChoice.Attack(actionMove))
  ...
  val lethalBt: BattleUnit = BattleUnit(player.id, bulbasaur , BattleChoice.Attack(lethalMove))
