...
/**
 * Factory used to construct Sequences of Trainers to be rendered on the Level screen
 */
object TrainerFactory:
  ...
  private def generateTrainers(grid: Grid, numberOfTrainer: Int): Seq[Trainer] =
    ...
    def _generateT(entityList: Seq[Trainer], numberOfTrainer: Int): Seq[Trainer] = entityList match
      case t if numberOfTrainer > 0 => _generateT(t :+ Trainer(id = idOpponent + getRandomOpponent, pos = grid.getRandomPos, pokemonList = PokemonFactory(2)), numberOfTrainer -1)
      case _ => entityList
